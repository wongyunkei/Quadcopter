...
bool Quaternion::Update(){
	if(_mOmega->getIsValided()){
		...
		if(_mAcceleration->getIsValided() && _mAcceleration->getAcc().norm() > Acceleration::Gravity * 0.9f && _mAcceleration->getAcc().norm() < Acceleration::Gravity * 1.1f){
			Vector4f q = T * _Quaternion;
			q.normalize();
			Eigen::Matrix<float, 3, 4> C = calcQuatToEulerMeasMatrix(q);
			Vector3f E = QuaternionToEuler(q);
			Vector3f angle = _mAcceleration->getAngle();
			angle[2] = E[2];
...
Eigen::Matrix<float, 3, 4> Quaternion::calcQuatToEulerMeasMatrix(Vector4f q){
	...
	Matrix3f R = QuaternionToMatrix(q).transpose();
...
Vector3f Quaternion::QuaternionToEuler(Vector4f q){
	...
	euler[1] = -asinf(MathTools::Trim(-1, 2 * (q[1] * q[3] - q[0] * q[2]), 1));
...
Vector3f Quaternion::MatrixToFixedAngles(Matrix3f R){
	...
	fixedAngles[0] = atan2f((float)-R(1,2), (float)R(2,2));
	fixedAngles[1] = asinf((float)R(0,2));
	fixedAngles[2] = atan2f((float)-R(0,1), (float)R(0,0));
	return fixedAngles;
}
...
////
//	_EulerKalman[0] = new Kalman(m(2,0), 0.000001f, 0.001f, 0.000001f);
//	_EulerKalman[1] = new Kalman(m(2,1), 0.000001f, 0.001f, 0.000001f);
//	_EulerKalman[2] = new Kalman(m(2,2), 0.000001f, 0.001f, 0.000001f);
